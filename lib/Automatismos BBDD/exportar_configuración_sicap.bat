...
::echo select version from aplicacion where codigo_aplicacion='0000000001'; >>version.txt
...
:Schema
Echo se obtiene el schema de base de datos
echo select  right('create table dbo.' + so1.name + '(' + '', 255 * ( abs( sign(sc1.colid - 1) - 1 ) ) )+ sc1.name + ' ' + st1.name + ' ' + >>schema_sql.txt
echo substring( '(' + rtrim( convert( char, sc1.length ) ) + ') ', 1, patindex('%char', st1.name ) * 10 ) +  substring( '(' + rtrim( convert( char, sc1.prec ) ) + ', ' + rtrim( >> schema_sql.txt
echo convert( char, sc1.scale ) ) + ') ' , 1, patindex('numeric', st1.name ) * 10 ) + >> schema_sql.txt
echo substring( 'NOT NULL', ( convert( int, convert( bit,( sc1.status & 8 ) ) ) * 4 ) + 1, >> schema_sql.txt
echo 8 * abs(convert(bit, (sc1.status & 0x80)) - 1 ) ) + right('identity ', 9 * convert(bit, (sc1.status & 0x80)) ) + >> schema_sql.txt
echo right(',', 5 * ( convert(int,sc2.colid) - convert(int,sc1.colid) ) ) + >> schema_sql.txt
echo right(' )' +  '' + '', 255 * abs( sign( ( convert(int,sc2.colid) - convert(int,sc1.colid) ) ) -1 ) ) >> schema_sql.txt
echo from    sysobjects so1,  syscolumns sc1, syscolumns sc2, systypes st1 >> schema_sql.txt
echo where so1.type = 'U' and sc1.id = so1.id and st1.usertype = sc1.usertype and sc2.id = sc1.id >> schema_sql.txt
echo and sc2.colid = (select max(colid) from syscolumns  where id = sc1.id) >> schema_sql.txt
echo order by so1.name, sc1.colid >> schema_sql.txt
echo go >> schema_sql.txt

isql -U%user% -P%password% -S %server%  >> log.txt -D%database% < schema_sql.txt >> schema_sql_2.txt
:: Se reconstruye el fichero
FINDSTR [2-n]  schema_sql_2.txt |FINDSTR /v row >> %version%.txt
echo go >> %version%.txt
