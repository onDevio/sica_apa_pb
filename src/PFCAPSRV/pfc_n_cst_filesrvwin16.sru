...
If MidA(as_NewDirectory, 2, 1) = ":" Then
	lc_Drive = LeftA(as_NewDirectory, 1)
Else
	lc_Drive = " "
End if
...
ls_yy = RightA(ls_date,4)
ls_mm = LeftA(ls_date,2)
if RightA(ls_mm,1) = "/" then
	ls_dd = MidA(ls_date,3,2)
	ls_mm = LeftA(ls_mm,1)
else
	ls_dd = MidA(ls_date,4,2)
end if
if RightA(ls_dd,1) = "/" then
	ls_dd = LeftA(ls_dd,1)
end if
...
Do While li_RC = 0
	lul_Attrib = AscA(lstr_FindData.ch_attrib)
...
li_Mon = Integer(LeftA(ls_Date, 2))
li_DD = Integer(MidA(ls_Date, 3, 2))
li_YY = Integer(RightA(ls_Date, 4))
...
li_HH = Integer(LeftA(ls_Time, 2))
li_MM = Integer(MidA(ls_Time, 3, 2))
li_SS = Integer(RightA(ls_Time, 2))
...
li_Drive = AscA(lower(LeftA(as_drive,1))) - 97
...
li_Drive = AscA(lower(LeftA(as_drive,1))) - 96
