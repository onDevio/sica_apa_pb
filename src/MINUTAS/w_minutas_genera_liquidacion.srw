HA$PBExportHeader$w_minutas_genera_liquidacion.srw
forward
global type w_minutas_genera_liquidacion from w_response
end type
type cb_1 from commandbutton within w_minutas_genera_liquidacion
end type
type cb_cancelar from commandbutton within w_minutas_genera_liquidacion
end type
type dw_liquid from u_dw within w_minutas_genera_liquidacion
end type
end forward
...
global w_minutas_genera_liquidacion w_minutas_genera_liquidacion
...
on w_minutas_genera_liquidacion.create
int iCurrent
call super::create
...
on w_minutas_genera_liquidacion.destroy
call super::destroy
destroy(this.cb_1)
...
dw_liquid.Setitem(1,'total_pagar',i_parametros.importe)
...
type cb_recuperar_pantalla from w_response`cb_recuperar_pantalla within w_minutas_genera_liquidacion
integer x = 1445
integer taborder = 70
end type

type cb_guardar_pantalla from w_response`cb_guardar_pantalla within w_minutas_genera_liquidacion
integer x = 1445
integer y = 988
end type
...
event clicked;string mensaje, forma_pago
...
if dw_liquid.getitemstring(1,'liquidar') = 'N' then
	...
	// Mejora SCP-351 y CBI-168
	// Comprobar la forma de pago del colegiado, y en caso de no coincidir con la puesta, avisar.
	SELECT forma_de_pago into :forma_pago FROM conceptos_remesables WHERE id_colegiado = :i_parametros.id_col;
	COMMIT;
	if forma_pago <> dw_liquid.getitemstring(1,'forma_pago') then
		messagebox(g_titulo,'ATENCI$$HEX1$$d300$$ENDHEX$$N: La forma de pago seleccionada ('+f_dame_formapago(dw_liquid.getitemstring(1,'forma_pago'))+') no coincide con la que tiene seleccionada el colegiado en su ficha ('+f_dame_formapago(forma_pago)+').~rVerifiquela.',StopSign!)
		return
	end if
	...
	i_parametros.importe_suma	= i_parametros.importe
	
	f_liquidacion(i_parametros)
