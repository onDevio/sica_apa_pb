HA$PBExportHeader$w_calcula_gastos_coaattg.srw
forward
global type w_calcula_gastos_coaattg from w_response
end type
type cb_2 from commandbutton within w_calcula_gastos_coaattg
end type
type dw_descuentos from u_dw within w_calcula_gastos_coaattg
end type
type cb_1 from commandbutton within w_calcula_gastos_coaattg
end type
type gb_1 from groupbox within w_calcula_gastos_coaattg
end type
type cb_impr from commandbutton within w_calcula_gastos_coaattg
end type
type dw_imprimir_ficha from u_dw within w_calcula_gastos_coaattg
end type
type tab_1 from tab within w_calcula_gastos_coaattg
end type
...
type dw_asociados from u_dw within w_calcula_gastos_coaattg
end type
type dw_resumen from u_dw within w_calcula_gastos_coaattg
end type
type dw_resumen_por_act from u_dw within w_calcula_gastos_coaattg
end type
type cb_limpiar from commandbutton within w_calcula_gastos_coaattg
end type
type gb_2 from groupbox within w_calcula_gastos_coaattg
end type
type gb_4 from groupbox within w_calcula_gastos_coaattg
end type
end forward
...
global w_calcula_gastos_coaattg w_calcula_gastos_coaattg
...
string id_fase,t_act,tipo_obra,tipo_gestion,destino_int
double sup_viv,sup_otros,sup_garaje,superficie,volumen,pem,porc_a,altura
string n_col,id_col,trabajo

id_fase=g_fases_busqueda.id_fase

select f.fase,f.tipo_trabajo,f.trabajo,f.tipo_gestion,f.destino_int,u.superficie,u.volumen,u.sup_viv,u.sup_garage,u.sup_otros,u.pem,u.altura,c.n_colegiado,c.id_colegiado,fc.porcen_a
into :t_act,:tipo_obra,:trabajo,:tipo_gestion,:destino_int,:superficie,:volumen,:sup_viv,:sup_garaje,:sup_otros,:pem,:altura,:n_col,:id_col,:porc_a
from fases f,fases_colegiados fc,colegiados c,fases_usos u where f.id_fase=:id_fase and f.id_fase=u.id_fase and fc.id_fase=f.id_fase and fc.id_col=c.id_colegiado;
...
on w_calcula_gastos_coaattg.create
int iCurrent
call super::create
...
on w_calcula_gastos_coaattg.destroy
call super::destroy
destroy(this.cb_2)
...
type cb_recuperar_pantalla from w_response`cb_recuperar_pantalla within w_calcula_gastos_coaattg
integer y = 1496
integer taborder = 0
end type

type cb_guardar_pantalla from w_response`cb_guardar_pantalla within w_calcula_gastos_coaattg
integer y = 1332
integer taborder = 0
end type
...
// cargo la estructura
for i=1 to 3
	st_cip_datos.tipo_act = dw_1.getitemstring(1, 'fase_'+string(i))
	...
	st_cip_datos.vol_edif = dw_1.GetItemnumber(1,'volumen')
	st_cip_datos.volumen = dw_1.GetItemNumber(1,'volumen')
	st_cip_datos.altura = dw_1.GetItemNumber(1,'altura')
	st_cip_datos.superficie = dw_1.getitemnumber(1, 'superficie')
	st_cip_datos.sup_viviendas = dw_1.getitemnumber(1, 'sup_viv')	
	st_cip_datos.sup_otros = dw_1.getitemnumber(1, 'sup_otros') + dw_1.getitemnumber(1, 'sup_garage')		
	st_cip_datos.pem = dw_1.getitemnumber(1, 'pem')
	st_cip_datos.destino_i = dw_1.getitemString(1, 'destino_int')
	st_cip_datos.tipo_reforma = dw_1.getitemString(1, 'tipo_reforma')
	st_cip_datos.num_viv = dw_1.getitemnumber(1, 'num_viv')
	if lower(dw_1.describe("funcionario.name")) = 'funcionario' then st_cip_datos.funcionario = dw_1.getitemString(1, 'funcionario')
	...
	if fila_insertada > 0 then
		...
		this.setitem(fila_insertada, 'tipo_actuacion', st_cip_datos.tipo_act)
		...
		formula_cip += 'DIP ('+string(i)+') = ' + st_cip_datos.formula +'~n'
		formula_desarrollo_cip += 'DIP ('+string(i)+') = '+ st_cip_datos.desarrollo +'~n'	
...
for ind=1 to 3
	...
	dw_resumen_por_act.setitem(ind, 'formula', 'MUSAAT ('+string(ind)+') = ' + st_musaat_datos.formula + '~n' +  'DIP ('+string(ind)+') = ' + st_cip_datos.formula + '~n' )
	dw_resumen_por_act.setitem(ind, 'desarrollo', 'MUSAAT ('+string(ind)+') = ' + st_musaat_datos.desarrollo + '~n' + 'DIP ('+string(ind)+') = ' + st_cip_datos.desarrollo +'~n'	)		
next

// Mostramos la formula y el desarrollo aplicados en el calculo de musaat
dw_resumen.setitem(1, 'formula', formula_mussat + '~n' +  formula_cip + '~n' )
dw_resumen.setitem(1, 'desarrollo', formula_desarrollo_mussat + '~n' +  formula_desarrollo_cip + '~n')				
...
choose case g_colegio
	case 'COAATTEB','COAATTGN'
		dw_descuentos.event csd_calcular_descuentos()		
	case 'COAATMCA','COAATLL'
		dw_descuentos.event csd_calcular_descuentos_nuevo()
...
integer height = 1028
integer taborder = 10
string dataobject = "d_calculo_gastos_tg"
boolean vscrollbar = false
boolean border = false
borderstyle borderstyle = stylebox!
boolean ib_isupdateable = false
...
int    i
string alta_baja,id,empresa,id_col,id_fase,mensaje,cobertura,sit
