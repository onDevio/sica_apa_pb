HA$PBExportHeader$w_tipos_incidencias_exp.srw
$PBExportComments$fases, incidenc
forward
global type w_tipos_incidencias_exp from w_mant_simple
end type
end forward
...
global w_tipos_incidencias_exp w_tipos_incidencias_exp

on w_tipos_incidencias_exp.create
call super::create
end on

on w_tipos_incidencias_exp.destroy
call super::destroy
...
mensaje += f_valida(dw_1,'descripcion','NOVACIO',g_idioma.of_getmsg('general.campo_incidencia','Debe especificar un valor en el campo Incidencia.') + cr) 
...
type cb_recuperar_pantalla from w_mant_simple`cb_recuperar_pantalla within w_tipos_incidencias_exp
end type

type cb_guardar_pantalla from w_mant_simple`cb_guardar_pantalla within w_tipos_incidencias_exp
end type

type dw_1 from w_mant_simple`dw_1 within w_tipos_incidencias_exp
integer width = 2071
string dataobject = "d_mant_tipos_incidencias_exp"
end type
...
// Comprobamos que no hayan colegiados que utilicen este c$$HEX1$$f300$$ENDHEX$$digo
	SELECT Count(*)  
	 INTO :num_registros
    FROM incidencias_exp  
   WHERE (incidencias_exp.codigo = :codigo) ;  
	...
	if num_registros > 0 then
		...
		messagebox(g_idioma.of_getmsg('general.suprimir',"Suprimir"), g_idioma.of_getmsg('general.borrar_registro',"No se puede borrar el registro. ~n") + &
		           g_idioma.of_getmsg('general.campo_incidencias_fases_tipo_inc_exp',"Existen incidencias o fases con este tipo de incidencia en el expediente.") , Exclamation!)
		return 0
	end if
...
type cb_anyadir from w_mant_simple`cb_anyadir within w_tipos_incidencias_exp
end type

type cb_borrar from w_mant_simple`cb_borrar within w_tipos_incidencias_exp
end type

type cb_ayuda from w_mant_simple`cb_ayuda within w_tipos_incidencias_exp
end type
