HA$PBExportHeader$w_tipos_registro.srw
$PBExportComments$fases
forward
global type w_tipos_registro from w_mant_simple
end type
end forward
...
global w_tipos_registro w_tipos_registro

on w_tipos_registro.create
call super::create
end on

on w_tipos_registro.destroy
call super::destroy
...
type cb_recuperar_pantalla from w_mant_simple`cb_recuperar_pantalla within w_tipos_registro
end type

type cb_guardar_pantalla from w_mant_simple`cb_guardar_pantalla within w_tipos_registro
end type

type dw_1 from w_mant_simple`dw_1 within w_tipos_registro
integer width = 2071
string dataobject = "d_tipos_registro"
end type
...
// Comprobamos que no hayan fases que utilicen este c$$HEX1$$f300$$ENDHEX$$digo
	SELECT Count(*)  
	 INTO :num_registros
    FROM fases
   WHERE (fases.tipo_registro = :codigo) ;  
	...
	if num_registros > 0 then
		...
		messagebox(g_idioma.of_getmsg('general.suprimir',"Suprimir"), g_idioma.of_getmsg('general.borrar_registro',"No se puede borrar el registro. ~n") + &
		           g_idioma.of_getmsg('general.msg_registros_tipo_registro',"Existen registros con este tipo de registro.") , Exclamation!)
		return 0
	end if
...
type cb_anyadir from w_mant_simple`cb_anyadir within w_tipos_registro
end type

type cb_borrar from w_mant_simple`cb_borrar within w_tipos_registro
end type

type cb_ayuda from w_mant_simple`cb_ayuda within w_tipos_registro
end type
