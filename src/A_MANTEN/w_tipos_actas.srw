HA$PBExportHeader$w_tipos_actas.srw
forward
global type w_tipos_actas from w_mant_simple
end type
end forward

global type w_tipos_actas from w_mant_simple
integer width = 2153
string title = "Mantenimiento de Tipos de Actas"
end type
global w_tipos_actas w_tipos_actas

on w_tipos_actas.create
call super::create
end on

on w_tipos_actas.destroy
call super::destroy
...
mensaje += f_valida(dw_1,'descripcion','NOVACIO',g_idioma.of_getmsg('general.campo_tipo_acta','Debe especificar un valor en el campo Tipo de Acta.')+cr)
...
type cb_recuperar_pantalla from w_mant_simple`cb_recuperar_pantalla within w_tipos_actas
end type

type cb_guardar_pantalla from w_mant_simple`cb_guardar_pantalla within w_tipos_actas
end type

type dw_1 from w_mant_simple`dw_1 within w_tipos_actas
integer width = 2071
string dataobject = "d_mant_tipos_actas"
end type
...
//
//else
//// Comprobamos que no hayan actas que utilicen este c$$HEX1$$f300$$ENDHEX$$digo
//	SELECT Count(*)  
//	 INTO :num_registros
//    FROM actas  
//   WHERE (actas.tipo_acta = :codigo) ;  
//
//	if num_registros > 0 then
//		dw_1.ScrollToRow(fila)
//		dw_1.SetColumn("codigo")
//		messagebox("Suprimir", "No se puede borrar el registro. ~n" + &
//		           "Existen actas con este tipo de acta." , Exclamation!)
//		return 0
//	end if
//
//end if
...
type cb_anyadir from w_mant_simple`cb_anyadir within w_tipos_actas
end type

type cb_borrar from w_mant_simple`cb_borrar within w_tipos_actas
end type

type cb_ayuda from w_mant_simple`cb_ayuda within w_tipos_actas
end type
