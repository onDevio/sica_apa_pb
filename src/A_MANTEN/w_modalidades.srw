HA$PBExportHeader$w_modalidades.srw
$PBExportComments$fases
forward
global type w_modalidades from w_mant_simple
end type
end forward

global type w_modalidades from w_mant_simple
integer width = 2153
string title = "Mantenimiento de Modalidades"
end type
global w_modalidades w_modalidades

on w_modalidades.create
call super::create
end on

on w_modalidades.destroy
call super::destroy
...
type cb_recuperar_pantalla from w_mant_simple`cb_recuperar_pantalla within w_modalidades
end type

type cb_guardar_pantalla from w_mant_simple`cb_guardar_pantalla within w_modalidades
end type

type dw_1 from w_mant_simple`dw_1 within w_modalidades
integer width = 2053
string dataobject = "d_mant_modalidades"
end type
...
fila = dw_1.GetRow()
codigo = dw_1.GetItemString(fila, "cod_modalidad")
...
// Comprobamos que no hayan fases que utilicen este c$$HEX1$$f300$$ENDHEX$$digo
	SELECT Count(*)  
	 INTO :num_registros
    FROM fases  
   WHERE (fases.modalidad = :codigo) ;  

	if num_registros > 0 then
		dw_1.ScrollToRow(fila)
		dw_1.SetColumn("cod_modalidad")
		messagebox(g_idioma.of_getmsg('general.suprimir',"Suprimir"), g_idioma.of_getmsg('general.borrar_registro',"No se puede borrar el registro. ~n") + &
		           g_idioma.of_getmsg('general.msg_modalidad',"Existen fases con esta modalidad.") , Exclamation!)
		return 0
	end if
...
IF GetItemStatus(currentrow,0,Primary!)=New! or GetItemStatus(currentrow,0,Primary!)=NewModified! THEN
	Object.cod_modalidad.Edit.DisplayOnly = "No"
ELSE	
	Object.cod_modalidad.Edit.DisplayOnly = "Yes"
END IF	

end event

type cb_anyadir from w_mant_simple`cb_anyadir within w_modalidades
end type

type cb_borrar from w_mant_simple`cb_borrar within w_modalidades
end type

type cb_ayuda from w_mant_simple`cb_ayuda within w_modalidades
end type
