HA$PBExportHeader$w_tipos_incidencias_familias.srw
$PBExportComments$Incidencias
forward
global type w_tipos_incidencias_familias from w_mant_simple
end type
end forward
...
integer width = 3031
integer height = 1492
string title = "Mantenimiento de Familias de incidencias"
end type
global w_tipos_incidencias_familias w_tipos_incidencias_familias

on w_tipos_incidencias_familias.create
call super::create
end on

on w_tipos_incidencias_familias.destroy
call super::destroy
...
mensaje += f_valida(dw_1,'descripcion','NOVACIO',g_idioma.of_getmsg('general.campo_descripcion_familia','Debe especificar un valor en el campo Descripci$$HEX1$$f300$$ENDHEX$$n de la familia.') + cr) 
...
type cb_recuperar_pantalla from w_mant_simple`cb_recuperar_pantalla within w_tipos_incidencias_familias
end type

type cb_guardar_pantalla from w_mant_simple`cb_guardar_pantalla within w_tipos_incidencias_familias
end type

type dw_1 from w_mant_simple`dw_1 within w_tipos_incidencias_familias
integer width = 2949
string dataobject = "d_mant_tipos_familias"
end type
...
// Comprobamos que no hayan colegiados que utilicen este c$$HEX1$$f300$$ENDHEX$$digo
	SELECT Count(*)  
	 INTO :num_registros
    FROM incidencias_expedientes  
   WHERE (incidencias_expedientes.tipo_familia = :codigo) ;  
	...
	if num_registros > 0 then
		...
		messagebox(g_idioma.of_getmsg('general.suprimir',"Suprimir"), g_idioma.of_getmsg('general.borrar_registro',"No se puede borrar el registro. ~n") + &
		           g_idioma.of_getmsg('general.campo_incidencias_tipo_fam',"Existen tipos de incidencias este tipo de familia.") , Exclamation!)
		return 0
	end if
...
type cb_anyadir from w_mant_simple`cb_anyadir within w_tipos_incidencias_familias
end type

type cb_borrar from w_mant_simple`cb_borrar within w_tipos_incidencias_familias
end type

type cb_ayuda from w_mant_simple`cb_ayuda within w_tipos_incidencias_familias
end type
