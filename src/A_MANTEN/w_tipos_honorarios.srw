HA$PBExportHeader$w_tipos_honorarios.srw
$PBExportComments$fases_minutas, fases_honorarios
forward
global type w_tipos_honorarios from w_mant_simple
end type
end forward
...
global w_tipos_honorarios w_tipos_honorarios

on w_tipos_honorarios.create
call super::create
end on

on w_tipos_honorarios.destroy
call super::destroy
...
type cb_recuperar_pantalla from w_mant_simple`cb_recuperar_pantalla within w_tipos_honorarios
end type

type cb_guardar_pantalla from w_mant_simple`cb_guardar_pantalla within w_tipos_honorarios
end type

type dw_1 from w_mant_simple`dw_1 within w_tipos_honorarios
integer width = 1893
string dataobject = "d_tipos_honorarios"
end type
...
// Comprobamos que no hayan fases de minutas que utilicen este c$$HEX1$$f300$$ENDHEX$$digo
	SELECT Count(*)  
	 INTO :num_registros
    FROM fases_minutas
   WHERE (fases_minutas.concepto_honos = :descripcion) ;  
	...
	if num_registros > 0 then
		...
		messagebox(g_idioma.of_getmsg('general.suprimir',"Suprimir"), g_idioma.of_getmsg('general.borrar_registro',"No se puede borrar el registro. ~n") + &
		           g_idioma.of_getmsg('general.msg_minutas_tipo_honorario',"Existen minutas con este tipo de honorario.") , Exclamation!)
		return 0
	end if
end if
...
type cb_anyadir from w_mant_simple`cb_anyadir within w_tipos_honorarios
end type

type cb_borrar from w_mant_simple`cb_borrar within w_tipos_honorarios
end type

type cb_ayuda from w_mant_simple`cb_ayuda within w_tipos_honorarios
end type
