HA$PBExportHeader$w_calculo_formulas.srw
$PBExportComments$objetos b$$HEX1$$e100$$ENDHEX$$sicos
forward
global type w_calculo_formulas from w_response
end type
type dw_variables from u_dw within w_calculo_formulas
end type
type cb_aceptar from u_cb within w_calculo_formulas
end type
type cb_cancelar from u_cb within w_calculo_formulas
end type
type cb_recalcular from u_cb within w_calculo_formulas
end type
type cbx_disenyo_avanzado from u_cbx within w_calculo_formulas
end type
end forward
...
cbx_disenyo_avanzado cbx_disenyo_avanzado
end type
global w_calculo_formulas w_calculo_formulas
...
this.cbx_disenyo_avanzado=create cbx_disenyo_avanzado
...
this.Control[iCurrent+5]=this.cbx_disenyo_avanzado
end on

on w_calculo_formulas.destroy
call super::destroy
destroy(this.dw_variables)
destroy(this.cb_aceptar)
destroy(this.cb_cancelar)
destroy(this.cb_recalcular)
destroy(this.cbx_disenyo_avanzado)
end on
...
if ib_dw_variables_personal then
	...
	cbx_disenyo_avanzado.visible = false
...
type cb_recuperar_pantalla from w_response`cb_recuperar_pantalla within w_calculo_formulas
integer taborder = 50
end type

type cb_guardar_pantalla from w_response`cb_guardar_pantalla within w_calculo_formulas
end type
...
integer height = 68
boolean bringtotop = true
string text = "Controles avanzados"
end type
...
for fila = 1 to dw_variables.rowcount()
	dw_variables.setitem(fila,'disenyo_avanzado',modo)
next
