...
if g_usar_idioma = "S" then
	g_idioma.of_cambia_textos( lm_dw )
end if
//////////////////////////////////////////////////////////////////////////////
...
If gnv_app.of_GetMicrohelp() Then
	...
	if IsNull (ls_microhelp) or LenA(Trim(ls_microhelp))=0 then
		ls_microhelp = ''	
	end if
...
// The error message.
ls_message = "Error de base de datos.~r~n~r~n~r~n" + &
	"C$$HEX1$$f300$$ENDHEX$$digo de error:  " + String (sqldbcode) + "~r~n~r~n" + &
	"Mensaje de error:~r~n" + sqlerrtext

if g_usar_idioma = "S" then
	ls_message = "Error de base de datos.~r~n~r~n~r~n" + &
		"C$$HEX1$$f300$$ENDHEX$$digo de error:  %s~r~n~r~n" + &
		"Mensaje de error:~r~n%s"
	string params[]
	params[1] = String(sqldbcode)
	params[2] = sqlerrtext
	ls_message = g_idioma.of_getmsg( "pfc.dberror", params, ls_message)
end if
