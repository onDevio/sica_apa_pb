HA$PBExportHeader$w_siniestros_tipo_estado_obra.srw
forward
global type w_siniestros_tipo_estado_obra from w_mant_simple
end type
end forward
...
integer x = 214
integer y = 221
string title = "Mantenimiento de Tipo de Estado de Obra"
end type
global w_siniestros_tipo_estado_obra w_siniestros_tipo_estado_obra

on w_siniestros_tipo_estado_obra.create
call super::create
end on

on w_siniestros_tipo_estado_obra.destroy
call super::destroy
...
type dw_1 from w_mant_simple`dw_1 within w_siniestros_tipo_estado_obra
integer width = 2976
string dataobject = "d_estado_obra"
end type
...
// Comprobamos que no hayan siniestros que utilicen este c$$HEX1$$f300$$ENDHEX$$digo
   ...
   WHERE (fases_siniestros.tipo_estado_obra  = :codigo) ;  

	if num_registros > 0 then
		dw_1.ScrollToRow(fila)
		dw_1.SetColumn("codigo")
		messagebox("Suprimir", "No se puede borrar el registro. ~n" + &
		           "Existen siniestros con este tipo de estados de obra." , Exclamation!)
		return 0
	end if
...
type cb_anyadir from w_mant_simple`cb_anyadir within w_siniestros_tipo_estado_obra
end type

type cb_borrar from w_mant_simple`cb_borrar within w_siniestros_tipo_estado_obra
end type

type cb_ayuda from w_mant_simple`cb_ayuda within w_siniestros_tipo_estado_obra
end type
