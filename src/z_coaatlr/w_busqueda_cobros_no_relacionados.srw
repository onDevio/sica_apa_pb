HA$PBExportHeader$w_busqueda_cobros_no_relacionados.srw
forward
global type w_busqueda_cobros_no_relacionados from w_busqueda
end type
type dw_3 from u_dw within w_busqueda_cobros_no_relacionados
end type
end forward
...
global w_busqueda_cobros_no_relacionados w_busqueda_cobros_no_relacionados

type variables
string sql
end variables

on w_busqueda_cobros_no_relacionados.create
int iCurrent
call super::create
...
on w_busqueda_cobros_no_relacionados.destroy
call super::destroy
destroy(this.dw_3)
end on
...
type cb_recuperar_pantalla from w_busqueda`cb_recuperar_pantalla within w_busqueda_cobros_no_relacionados
end type

type cb_guardar_pantalla from w_busqueda`cb_guardar_pantalla within w_busqueda_cobros_no_relacionados
end type

type sle_1 from w_busqueda`sle_1 within w_busqueda_cobros_no_relacionados
end type

type dw_2 from w_busqueda`dw_2 within w_busqueda_cobros_no_relacionados
...
type st_1 from w_busqueda`st_1 within w_busqueda_cobros_no_relacionados
end type

type st_2 from w_busqueda`st_2 within w_busqueda_cobros_no_relacionados
boolean visible = false
integer y = 552
integer width = 777
...
type cb_1 from w_busqueda`cb_1 within w_busqueda_cobros_no_relacionados
integer y = 1776
integer taborder = 70
end type
...
type cb_2 from w_busqueda`cb_2 within w_busqueda_cobros_no_relacionados
integer y = 1776
integer taborder = 80
boolean cancel = true
end type

type dw_1 from w_busqueda`dw_1 within w_busqueda_cobros_no_relacionados
integer x = 37
integer y = 212
integer width = 2894
integer height = 1504
integer taborder = 60
string dataobject = "d_cobros_no_relacionados_busqueda"
end type
...
string dataobject = "d_avisos_consulta_busqueda"
...
string  id_col, id_cliente, pob, apellidos
...
if do_case then
	choose case dwo.name
		case "b_1"
			...
			f_sql('fases.n_expedi','LIKE','n_expediente',dw_3,sql_nuevo,g_tipo_base_datos,'')
			f_sql('fases.n_registro','LIKE','n_registro',dw_3,sql_nuevo,g_tipo_base_datos,'')
			f_sql('fases_minutas.n_aviso','LIKE','n_aviso',dw_3,sql_nuevo,g_tipo_base_datos,'')
...
//			f_sql('fases_clientes.id_cliente','LIKE','id_cliente',dw_3,sql_nuevo,g_tipo_base_datos,'') 
		...
		case "b_busca_colegiado"
			...
			if NOT f_es_vacio(id_col)  then
				...
				SELECT apellidos into :apellidos from colegiados where id_colegiado = :id_col;
				this.setitem(1, 'nombre_colegiado', apellidos)
			end if
		...
		case "b_busca_cliente"
			...
			if NOT f_es_vacio(id_cliente)  then
				...
				SELECT apellidos into :apellidos from clientes where id_cliente = :id_cliente;
				this.setitem(1, 'nombre_promotor', apellidos)
			end if
