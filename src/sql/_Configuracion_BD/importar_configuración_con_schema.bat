...
echo Introduzca la ruta de base de datos \src\resources\datos\_configuracion_BD
set /p ruta_bd= 
if "%ruta_bd%"=="" set ruta_bd= %ruta_base%

echo Introduzca la versión del schema de base de datos (ejemplo: 2.00.08.sql):
set /p schema= 
if "%schema%"=="" set schema= schema.sql 
echo Cargando schema de base de datos  %ruta_bd%%schema% 
isql -U%user% -P%password% -S %server%  >> log.txt -D%database% < %ruta_bd%%schema%
