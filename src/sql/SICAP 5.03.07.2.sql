...
--Script de actualización de las tarifas de MUSAAT 2012
--Se agrega nuevo mínimo para la tabla D, con la creación de una nueva tarifa F
...
-- SCP-1906 Se agrega nuevo tipo de actuación  de forma condicional si aún no existe
declare @aux int
select @aux = count(*) from t_fases where c_t_fase  IN ('76','79')
if @aux = 0
begin
	insert t_fases (c_t_fase, d_t_descripcion, grupo) values('76', 'CÉDULA DE HABITABILIDAD', '4')
end
-- Se agrega nuevo tipo de actuación 
insert t_fases (c_t_fase, d_t_descripcion, grupo) values('77', 'INSPECCIONES TÉCNICAS DE EDIFICIOS (ITE)', '4')

-- Se añade a la tabla musaat_coef_c los tipos de actuacion 76 y 79 utilizado para Cédulas de habitabilidad y 77 para inspecciones técnicas (Añadida verificación previa tras error detectado)
select @aux = count(*) from musaat_coef_c where tipoact = '76'
if @aux = 0
begin
...
select @aux = count(*) from musaat_coef_c where tipoact = '79'
if @aux = 0
begin
	insert musaat_coef_c  values ('79', '*', 0, 'F', -1, 9999999, '19000101', '20500101')
end

--Modificación en la tabla del coeficiente G 
...
update musaat_coef_g set coef = 2.954, coef_plus = 3.693 where musaat_coef_g.cobertura = 900000 
...
update colegios set musaat_coef_malus = 0.92 where cod_colegio in('36','41')
...
update colegios set musaat_coef_malus = 1 where cod_colegio in('09','11','24','27','28','29','51')

declare @colegio varchar(10)
select @colegio = texto from var_globales where nombre = 'COLEGIO'

--Recargos
if @colegio = 'COAATCC' or @colegio = 'COAATTGN' or @colegio = 'COAATTFE' 
begin
  update var_globales set numero = 1.02 where nombre = 'g_col_coef_musaat'
end

if @colegio = 'COAATAVI' or @colegio = 'COAATLR' or @colegio = 'COAATMU' or @colegio = 'COAATZ'
begin
...
if @colegio = 'COAATA' or @colegio = 'COAATMCA' 
begin
